...
#include <M5Stack.h>
#include <SPIFFS.h>
String file_name1 = "/data/mqtt_save.json"; //Sets the location and name of the file to be operated on.  设置被操作的文件位置和名称
String file_name2 = "/data/mqtt_settings.json"; //Sets the location and name of the file to be operated on.  设置被操作的文件位置和名称
String file_name3 = "/data/mqtt_param.json"; //Sets the location and name of the file to be operated on.  设置被操作的文件位置和名称
...
void setup() {
  ...
  if(SPIFFS.begin()){ // Start SPIFFS, return 1 on success.  启动闪存文件系统,若成功返回1
    ...
    //Write operation
    File dataFile = SPIFFS.open(file_name1, "w");  // Create a File object dafa File to write information to file_name in the SPIFFS.  建立File对象dafaFile用于向SPIFFS中的file_name写入信息
